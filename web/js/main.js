...
var moving = false; // whether the player is moving
var averageX = new Array();
var command = "WAITING";
var commandTime = Date.now();

function calcAverage() { 
  var sum = 0;
  for (var i = 0; i < averageX.length; i++) {
    sum = sum + averageX[i];
  }
  if (averageX.length === 0) { 
    return 0;
  }
  else {
    var avg = Math.round(sum / averageX.length);
    return avg;
  }
}
...
//    qs.server = window.getQueryString('server') || 'localhost';					    // name of spacebrew server
    qs.server = window.getQueryString('server') || 'sandbox.spacebrew.cc';					    // name of spacebrew server
...
function setup (){
  ...
  // publish state of each player
  //sb.connection.addPublish( "moving", "boolean");

  // get stop/go commands
  sb.connection.addSubscribe("stop/go", "boolean");

  sb.connection.onBooleanMessage = function onBoolean( name, value ) {
    if (name === "stop/go") {
      if (value === true) { 
        command = "RUN";
      }
    ...
    for (var p in parts) {
      ...
      if (state[sensor][part] != new_state) {
        ...
        if (sb.connected) {
          ...
          var threshold = 50; // set this value for checking motion
          if ((sensor == 'accel') && (part == 'x')) { 
            var currentValue = state[sensor][part];
            var currentAverage = calcAverage();
            if (Math.abs(currentValue - currentAverage) > threshold) {
              $('input#average').val(currentAverage);
              moving = true;
              if ((command === "FREEZE") && ((Date.now() - commandTime) > 2000)) {
            ...
            averageX.unshift(currentValue);
            if (averageX.length > 10) {
              averageX.length = 10;
            }
          }
          sb.connection.send("moving", "boolean", moving);
          new_data = true;
        }
      }
    }
